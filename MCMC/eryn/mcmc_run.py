...
def waveform_check(*params, **kwargs):
    """
...
def llike(*params, **like_kwargs):
    ...
    signal_prop_t = waveform_check(*params,**llike_kwargs)
    signal_prop_f = FFT(signal_prop_t)

    data_f = llike_kwargs["data_stream"]
    variance_noise_f = llike_kwargs["variance_noise_f"]
...
h_true_t = waveform_check(*true_params,**kwargs)
...
start_f = true_params[1]*(1 + delta_f/true_params[0] * np.random.uniform(1,2,size = nwalkers).reshape(nwalkers,1))
start_fdot = true_params[2]*(1 + delta_dotf/true_params[0] * np.random.uniform(1,2,size = nwalkers).reshape(nwalkers,1))
