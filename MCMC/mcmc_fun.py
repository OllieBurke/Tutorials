...
def MCMC_run(data_f, t, variance_noise_f,
    ...
    for i in range(1, Ntotal):
        ...
        ####
        # Step 3. Perform accept_reject call
        ####
        # breakpoint()
        if accept_reject(lp_prop, lp_prev) == 1:  # Accept
            f_chain.append(f_prop)    # accept a_{prop} as new sample
            a_chain.append(a_prop)    # accept a_{prop} as new sample
            fdot_chain.append(fdot_prop)    # accept a_{prop} as new sample
            accept_reject_count.append(1)
            lp_store = lp_prop  # Overwrite lp_store
        else:  # Reject
            a_chain.append(a_chain[i - 1])
            f_chain.append(f_chain[i - 1])  # Store previous one
    ...
    return a_chain,f_chain, fdot_chain,lp